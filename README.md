...
* **节能与生命周期管理**：接入系统事件协调中心，息屏与锁屏时自动休眠轮询 Timer。
...
### 11. 🛡️ 统一权限管理看板 (Permission Center)
...
## 🔐 统一权限中心与透明度 (Permission Center)

CalmBar 严格遵循 macOS 最小权限原则，所有功能均在本地运行，零网络上传。你在 CalmBar 设置中可随时进入 **「权限安全」** 看板查看与管理各项权限：

1. **辅助功能权限 (Accessibility)**：
   * 用于系统级拦截并翻转外接鼠标滚轮事件 (`CGEventTap`)，以及办公软件防离开微动仿真。
   * 前往 **系统设置 -> 隐私与安全性 -> 辅助功能** 授权。
2. **SMC 特权助手 (Fan Control & Battery Helper)**：
   * 用于向系统 SMC 寄存器写入风扇目标转速及电池充电控制。
   * 点击 CalmBar 面板中的 **「一键激活」** 按钮，输入开机密码即可安装特权服务。
3. **屏幕录制权限 (Screen Capture - 仅选区 OCR 需要)**：
   * 用于屏幕选区截屏并调用 Apple Vision 深度学习引擎离线识别文字与二维码。
4. **完全磁盘访问权限 (Full Disk Access - 可选)**：
   * 用于应用卸载与开发工具链清理时完整扫描受系统保护的偏好与缓存目录。
