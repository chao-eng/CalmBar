...
* **🕹️ 操作说明**：点击菜单栏图标展开面板，顶部卡片直观显示 CPU/GPU 实时温度与风扇转速，在风扇模块中切换模式或滑动滑块即可即时调速。
...
* **🕹️ 操作说明**：在菜单栏面板点击「菜单收纳」的「展开/折叠」按钮，或直接点击菜单栏收纳分隔符即可收起或展开多余图标。
...
* **🕹️ 操作说明**：开启「Music 拦截」开关后，系统误触唤起 Apple Music 时将被无感拦截，可在偏好设置中指定替换启动的第三方播放器。
...
* **🕹️ 操作说明**：点击「防休眠」开关开启长亮状态；点击开关旁边的 ⏱️ 时钟图标可选择 5分钟 ~ 5小时 的定时自动停止选项。
...
* **🕹️ 操作说明**：开启「充电保护」开关即可将电量锁定在设定上限（默认 80%）并转为旁路供电；点击 ⚡ 闪电按钮可一键临时充至 100%。
...
* **🕹️ 操作说明**：点击面板中的 **「应用去隔离」** 打开设置页，直接将打不开的应用拖拽进虚线框即可自动完成修复。
